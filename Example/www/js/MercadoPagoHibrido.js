...
.controller('MpPaymentMethodSearch2Ctrl', function($scope, MercadoPagoService, $state, $stateParams, $ionicHistory, $rootScope){
  ...
  $scope.selectedPaymentMethod = function(selectedPaymentMethod) {
    if ($stateParams.flavour == 1){
        ...
        datos.push(selectedPaymentMethod);
        datos.push($scope.cardIssuer);
        datos.push($scope.installments);
...
.controller('MpCheckoutCtrl', function($scope, MercadoPagoService, $state, $stateParams, $ionicHistory, $rootScope) {
  ...
  $scope.getImage = function(pm) { //agrego la imagen a la paymentMethod
    if (pm != undefined) {
        var id = pm.id;
        if (id == "redlink_bank_transfer" || id == "redlink_atm") {
            id = "redlink";
        }
    ...
    .get(function(data) {
      ...
      errorNum = 0;
        ...
        if (error.status == -1) {
            if (errorNum < 3) {
                errorNum++;
                $scope.getInstallments($stateParams.paymentMethod.id, $stateParams.issuer.id, $scope.prefIdTotal);
          } else {
                alert("Intente devuelta");
                errorNum = 0;
        }
...
.controller('MpCongratsCtrl', function($scope, MercadoPagoService, $state, $stateParams, $ionicHistory, $rootScope){
  ...
  $scope.getImage = function(){ //buscar imagenes de los pm
    ...
    if (pm == "redlink_bank_transfer" || pm == "redlink_atm") {
        pm = "redlink";
    }
    for (a = 0; a < $rootScope.datos.payment_methods.length; a++) {
